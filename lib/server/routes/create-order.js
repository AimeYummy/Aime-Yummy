...
import { attachTelegramMessageId, createOrderRecord, releaseOrderStock, reserveOrderStock } from '../_store.js'
...
export default async function handler(req,res){
  ...
  try{
    ...
    let table=null
    if(body.tableId){
      const {data,error}=await supabase.from('tables').select('id,name,status,is_active').eq('id',body.tableId).eq('is_active',true).maybeSingle();
      if(error)throw error;
      if(!data)throw new Error('Meja sudah tidak aktif.');
      if(data.status!=='Kosong')throw new Error(`Meja ${data.name} sedang ${String(data.status).toLowerCase()}.`);
      table=data
      const {data:claimed,error:claimError}=await supabase.from('tables').update({status:'Terisi',updated_at:new Date().toISOString()}).eq('id',data.id).eq('status','Kosong').eq('is_active',true).select('id,name,status').maybeSingle()
      if(claimError)throw claimError
      if(!claimed)throw new Error('Meja baru saja dipakai pelanggan lain. Silakan pilih meja lain.')
    }
    ...
    try{
      ...
      if(table)await supabase.from('tables').update({status:'Kosong',updated_at:new Date().toISOString()}).eq('id',table.id).eq('status','Terisi')
      throw e
    }
