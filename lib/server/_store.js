...
async function deductOrderStock(order) {
  ...
  try {
    for (const item of items) {
      ...
      const { data: menu, error: readError } = await supabase.from('menu_items').select('id,name,stock,pcs_per_mika').eq('id', id).single()
      if (readError) throw readError
      const stock = Number(menu.stock || 0)
      if (stock < quantity) throw new Error(`Stok ${menu.name} tidak mencukupi`)
      const nextStock = stock - quantity
      const { error: updateError } = await supabase.from('menu_items').update({stock:nextStock,in_stock:nextStock>0,available:nextStock>0,updated_at:nowIso()}).eq('id', id)
      if (updateError) throw updateError
      touched.push({id,quantity,ppm:Number(menu.pcs_per_mika||1)})
      const {error:movementError}=await supabase.from('stock_movements').insert({menu_id:id,type:'OUT',quantity_mika:0,pcs_per_mika:Number(menu.pcs_per_mika||1),quantity_pcs:quantity,reference_type:'order',reference_id:order.orderId,description:`Penjualan ${order.orderId}`,user_id:null})
      if(movementError)throw movementError
    }
  } catch(error) {
    for(const row of touched){
      await supabase.from('menu_items').select('stock').eq('id',row.id).single().then(async ({data})=>{
        if(data) await supabase.from('menu_items').update({stock:Number(data.stock||0)+row.quantity,in_stock:true,available:true,updated_at:nowIso()}).eq('id',row.id)
      })
    }
    throw error
  }
