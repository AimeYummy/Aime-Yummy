...
function buildOrderRow(order = {}) {
  ...
  if (order.stockDeducted !== undefined) row.stock_deducted = Boolean(order.stockDeducted)
  if (order.tableId !== undefined) row.table_id = order.tableId || null
...
async function deductOrderStock(order) {
  ...
  try {
    for (const item of items) {
      const id = String(item.id || item.menuId || '').trim()
      const quantity = Math.max(0, Math.floor(Number(item.qty ?? item.quantity ?? 0)))
      if (!id || quantity <= 0) continue

      const { data: menu, error: readError } = await supabase
        .from('menu_items')
        .select('id,name,stock,pcs_per_mika,available')
        .eq('id', id)
        .single()
      if (readError) throw readError

      const stock = Number(menu.stock || 0)
      if (menu.available === false || stock < quantity) {
        throw new Error(`Stok ${menu.name} tidak mencukupi`)
      }
      ...
      touched.push({ id, quantity, ppm: Number(menu.pcs_per_mika || 1) })
      ...
      const { error: movementError } = await supabase
        ...
        .insert({
          ...
          type: 'OUT',
          quantity_mika: 0,
          pcs_per_mika: Number(menu.pcs_per_mika || 1),
    ...
    await rollbackStock(touched, order?.orderId, 'reserve-rollback')
    throw error
  }
}

async function rollbackStock(rows = [], referenceId = null, referenceType = 'rollback') {
  for (const row of rows) {
    ...
    const restoredStock = Number(data.stock || 0) + row.quantity
    await supabase
      .from('menu_items')
      .update({
        stock: restoredStock,
    ...
    await supabase.from('stock_movements').insert({
      ...
      type: 'IN',
      quantity_mika: 0,
      pcs_per_mika: row.ppm || 1,
      quantity_pcs: row.quantity,
      reference_type: referenceType,
      reference_id: referenceId ? String(referenceId) : null,
      description: referenceId ? `Pengembalian stok ${referenceId}` : 'Pengembalian stok',
      user_id: null,
    })
  }
}

// Reserve stock immediately when an order is created. The order then carries
// stockDeducted=true, so confirmation does not deduct the same stock twice.
export async function reserveOrderStock(order) {
  if (!order?.orderId) throw new Error('orderId is required to reserve stock')
  if (order.stockDeducted === false) return
  await deductOrderStock(order)
}

// Restore stock for an order that is cancelled/failed after it was reserved.
// Callers guard this operation with stockDeducted=true, making it idempotent at
// the application level and preventing normal duplicate refunds.
export async function releaseOrderStock(order) {
  ...
  for (const item of items) {
    ...
    const { data: menu, error } = await supabase
      .from('menu_items')
      .select('id,stock,pcs_per_mika')
      .eq('id', id)
      .single()
    if (error) throw error
    rows.push({ id, quantity, ppm: Number(menu.pcs_per_mika || 1) })
  }
  await rollbackStock(rows, order?.orderId, 'order-release')
}
