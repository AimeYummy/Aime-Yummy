 ...
 return <AdminPageShell title="Menu" subtitle="Satu sumber data untuk menu Admin dan website pembelian.">
  ...
  <div className="grid gap-5 xl:grid-cols-[430px_minmax(0,1fr)]">
   <form onSubmit={submit} className="rounded-[30px] border border-white/70 bg-white/65 p-5 shadow-sm backdrop-blur-xl dark:border-white/10 dark:bg-white/5 sm:p-6">
    <div className="mb-5 flex items-start justify-between"><div><div className="mb-2 flex items-center gap-2"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-indigo-100 text-indigo-600 dark:bg-indigo-500/15 dark:text-indigo-300"><AdminIcon name={editing?'edit':'plus'} size={17}/></span><h2 className="text-lg font-black text-slate-900 dark:text-white">{editing?'Edit Menu':'Tambah Menu'}</h2></div><p className="text-xs text-slate-400">Perubahan tersimpan langsung ke Supabase.</p></div>{editing?<button type="button" onClick={reset} className="text-xs font-black text-slate-400">Batal</button>:null}</div>
    <div className="space-y-4">
     ...
     <label><span className={label}>Harga</span><input className={input} type="number" min="1" value={form.price} onChange={e=>set('price',e.target.value)} required/></label>
     ...
     <label className="flex items-center gap-3 rounded-2xl bg-slate-50/80 px-4 py-3 text-sm font-bold text-slate-700 dark:bg-white/5 dark:text-slate-200"><input type="checkbox" checked={form.available} onChange={e=>set('available',e.target.checked)}/> Tersedia / tampil di website</label>
     <label className="flex items-center gap-3 rounded-2xl bg-slate-50/80 px-4 py-3 text-sm font-bold text-slate-700 dark:bg-white/5 dark:text-slate-200"><input type="checkbox" checked={form.hasVariant} onChange={e=>set('hasVariant',e.target.checked)}/> Menggunakan Variant</label>
     {form.hasVariant?<div className="rounded-2xl bg-slate-50/80 p-3 dark:bg-white/5"><div className="mb-3 flex items-center justify-between"><p className="text-xs font-black uppercase tracking-wide text-slate-500">Daftar Variant</p><button type="button" onClick={()=>set('variants',[...form.variants,{label:'',price:0}])} className="rounded-xl bg-indigo-600 px-3 py-2 text-xs font-black text-white">+ Tambah Variant</button></div><div className="space-y-2">{form.variants.map((v,i)=><div className="grid grid-cols-[1fr_110px_40px] gap-2" key={i}><input className={input} placeholder="Nama variant" value={v.label} onChange={e=>set('variants',form.variants.map((x,j)=>j===i?{...x,label:e.target.value}:x))}/><input className={input} type="number" min="0" value={v.price} onChange={e=>set('variants',form.variants.map((x,j)=>j===i?{...x,price:Number(e.target.value||0)}:x))}/><button type="button" className="rounded-2xl bg-rose-50 text-rose-600" onClick={()=>set('variants',form.variants.filter((_,j)=>j!==i))}><AdminIcon name="trash" size={15}/></button></div>)}</div></div>:null}
    </div>
    <button type="button" onClick={reset} className="mt-5 flex w-full items-center justify-center gap-2 rounded-2xl bg-emerald-600 px-4 py-3 text-sm font-black text-white shadow-sm transition hover:bg-emerald-700"><AdminIcon name="plus" size={16}/> Tambah Menu Baru</button>
    <div className="mt-2 flex gap-2"><button type="button" onClick={reset} className="flex-1 rounded-2xl border border-white/70 bg-white/75 px-4 py-3 text-sm font-black text-slate-700 dark:border-white/10 dark:bg-white/5 dark:text-slate-200">Bersihkan</button><button disabled={busy} className="flex-1 rounded-2xl bg-indigo-600 px-4 py-3 text-sm font-black text-white disabled:opacity-60">{busy?'Menyimpan...':editing?'Simpan Perubahan':'Save'}</button></div>
   </form>
   <div className="rounded-[30px] border border-white/70 bg-white/65 p-5 shadow-sm backdrop-blur-xl dark:border-white/10 dark:bg-white/5 sm:p-6"><div className="mb-4 flex items-center justify-between"><div><h2 className="text-lg font-black text-slate-900 dark:text-white">Daftar Menu</h2><p className="text-xs text-slate-400">{items.length} menu dari database</p></div><button type="button" onClick={load} className="rounded-xl bg-slate-100 p-2.5 dark:bg-white/10" aria-label="Refresh"><AdminIcon name="refresh" size={15}/></button></div><div className="overflow-x-auto"><table className="w-full min-w-[720px] text-left text-sm"><thead><tr className="border-b text-[10px] font-black uppercase tracking-wider text-slate-400"><th className="px-3 py-3">Menu</th><th className="px-3 py-3">Kategori</th><th className="px-3 py-3">Harga</th><th className="px-3 py-3">Stok</th><th className="px-3 py-3">Status</th><th className="px-3 py-3">Aksi</th></tr></thead><tbody>{items.map(item=><tr key={item.id} className="border-b last:border-0"><td className="px-3 py-3"><div className="flex items-center gap-3"><img src={item.imageUrl} alt="" className="h-10 w-10 rounded-xl object-cover"/><div><p className="font-black">{item.name}</p>{item.hasVariant?<p className="text-[10px] text-indigo-500">{item.variants?.length||0} variant</p>:null}</div></div></td><td className="px-3 py-3">{item.category}</td><td className="px-3 py-3 font-black">{money(item.price)}</td><td className="px-3 py-3 font-black">{item.stock} pcs</td><td className="px-3 py-3"><span className={`rounded-full px-2 py-1 text-[10px] font-black ${item.available?'bg-emerald-100 text-emerald-700':'bg-slate-100 text-slate-500'}`}>{item.available?'Aktif':'Nonaktif'}</span></td><td className="px-3 py-3"><div className="flex gap-1"><button type="button" onClick={()=>edit(item)} className="rounded-xl p-2 text-indigo-600"><AdminIcon name="edit" size={16}/></button><button type="button" onClick={()=>remove(item)} className="rounded-xl p-2 text-rose-600"><AdminIcon name="trash" size={16}/></button></div></td></tr>)}{!items.length?<tr><td colSpan="6" className="px-3 py-12 text-center text-sm font-semibold text-slate-400">Belum ada menu.</td></tr>:null}</tbody></table></div></div>
  </div>
 </AdminPageShell>
}
