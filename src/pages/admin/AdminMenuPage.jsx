...
export default function AdminMenuPage(){
 ...
 const remove=async item=>{if(!window.confirm(`Hapus menu "${item.name}" dari website?`))return;setBusy(true);setError('');try{await deleteMenuItem(item.id);await load();await refreshCustomer();setNotice('Menu berhasil dihapus dari database dan website.')}catch(e){setError(e.message||'Gagal menghapus menu.')}finally{setBusy(false)}}
 return <AdminPageShell title="Menu" subtitle="Satu sumber data untuk menu Admin dan website pembelian.">
  ...
  <div className="admin-menu-layout grid min-w-0 grid-cols-1 gap-4 lg:grid-cols-[390px_minmax(0,1fr)] xl:grid-cols-[430px_minmax(0,1fr)]">
   <form onSubmit={submit} className="admin-form-card rounded-[30px] border border-white/70 bg-white/65 p-4 shadow-sm backdrop-blur-xl dark:border-white/10 dark:bg-white/5 sm:p-5">
    ...
    <div className="space-y-3">
     ...
     {form.hasVariant?<div className="admin-variant-box rounded-2xl bg-slate-50/80 p-2.5 dark:bg-white/5"><div className="mb-2 flex min-w-0 items-center justify-between gap-2"><p className="text-xs font-black uppercase tracking-wide text-slate-500">Daftar Variant</p><button type="button" onClick={()=>set('variants',[...form.variants,{label:'',price:0}])} className="admin-add-variant-btn">+ Tambah Variant</button></div><div className="space-y-2">{form.variants.map((v,i)=><div className="admin-variant-row-mobile grid grid-cols-[minmax(0,1fr)_92px_36px] gap-2" key={i}><input className={input} placeholder="Nama variant" value={v.label} onChange={e=>set('variants',form.variants.map((x,j)=>j===i?{...x,label:e.target.value}:x))}/><input className={`${input} admin-money-input`} type="number" min="0" inputMode="numeric" value={v.price} onChange={e=>set('variants',form.variants.map((x,j)=>j===i?{...x,price:Number(e.target.value||0)}:x))}/><button type="button" className="admin-variant-delete-btn" onClick={()=>set('variants',form.variants.filter((_,j)=>j!==i))}><AdminIcon name="trash" size={15}/></button></div>)}</div></div>:null}
    </div>
    <div className="admin-form-actions-mobile mt-4 grid grid-cols-2 gap-2 sm:flex sm:justify-end">
      <button type="button" onClick={reset} className="admin-secondary-btn sm:w-auto">Bersihkan</button>
      <button disabled={busy} className="admin-primary-btn sm:w-auto">{busy?'Menyimpan...':editing?'Simpan Perubahan':'Save'}</button>
    </div>
   </form>
   <div className="admin-menu-list-panel min-w-0 rounded-[30px] border border-white/70 bg-white/65 p-4 shadow-sm backdrop-blur-xl dark:border-white/10 dark:bg-white/5 sm:p-6"><div className="mb-4 flex items-center justify-between"><div><h2 className="text-lg font-black text-slate-900 dark:text-white">Daftar Menu</h2><p className="text-xs text-slate-400">{items.length} menu dari database</p></div><button type="button" onClick={load} className="rounded-xl bg-slate-100 p-2.5 dark:bg-white/10" aria-label="Refresh"><AdminIcon name="refresh" size={15}/></button></div><div className="admin-horizontal-scroll" tabIndex={0} aria-label="Geser untuk melihat daftar menu"><table className="w-full min-w-[720px] text-left text-sm"><thead><tr className="border-b text-[10px] font-black uppercase tracking-wider text-slate-400"><th className="px-3 py-3">Menu</th><th className="px-3 py-3">Kategori</th><th className="px-3 py-3">Harga</th><th className="px-3 py-3">Stok</th><th className="px-3 py-3">Status</th><th className="px-3 py-3">Aksi</th></tr></thead><tbody>{items.map(item=><tr key={item.id} className="border-b last:border-0"><td className="px-3 py-3"><div className="flex items-center gap-3"><img src={item.imageUrl} alt="" className="h-10 w-10 rounded-xl object-cover"/><div><p className="font-black">{item.name}</p>{item.hasVariant?<p className="text-[10px] text-indigo-500">{item.variants?.length||0} variant</p>:null}</div></div></td><td className="px-3 py-3">{item.category}</td><td className="px-3 py-3 font-black">{money(item.price)}</td><td className="px-3 py-3 font-black">{item.stock} pcs</td><td className="px-3 py-3"><span className={`rounded-full px-2 py-1 text-[10px] font-black ${item.available?'bg-emerald-100 text-emerald-700':'bg-slate-100 text-slate-500'}`}>{item.available?'Tersedia':'Tidak tersedia'}</span></td><td className="px-3 py-3"><div className="flex gap-1"><button type="button" onClick={()=>edit(item)} className="rounded-xl p-2 text-indigo-600"><AdminIcon name="edit" size={16}/></button><button type="button" onClick={()=>remove(item)} className="rounded-xl p-2 text-rose-600"><AdminIcon name="trash" size={16}/></button></div></td></tr>)}{!items.length?<tr><td colSpan="6" className="px-3 py-12 text-center text-sm font-semibold text-slate-400">Belum ada menu.</td></tr>:null}</tbody></table></div></div>
  </div>
 </AdminPageShell>
}
