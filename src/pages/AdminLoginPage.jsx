...
export default function AdminLoginPage() {
  ...
  return (
    <div className="admin-login-app font-sans relative min-h-screen overflow-hidden bg-[radial-gradient(circle_at_top_right,_#fffaf8_0%,_#f5e9e8_48%,_#f8f3f0_100%)] px-4 py-8 text-slate-800 dark:bg-[radial-gradient(circle_at_top_right,_#251216_0%,_#1d1518_48%,_#140f10_100%)] dark:text-slate-100 sm:px-6">
      <div className="pointer-events-none absolute -right-24 -top-24 h-72 w-72 rounded-full bg-rose-800/15 blur-3xl" />
      <div className="pointer-events-none absolute -bottom-32 -left-24 h-80 w-80 rounded-full bg-purple-500/15 blur-3xl" />

      <button type="button" onClick={() => setDark(v => !v)} className="absolute right-4 top-4 z-20 flex h-10 w-10 items-center justify-center rounded-2xl border border-white/70 bg-white/70 text-slate-500 shadow-lg backdrop-blur-xl transition hover:-translate-y-0.5 hover:text-rose-800 dark:border-white/10 dark:bg-white/10 dark:text-slate-200 sm:right-6 sm:top-6" aria-label="Ubah tema">
      ...
      <div className="relative z-10 grid min-h-[calc(100vh-4rem)] place-items-center">
        <motion.div initial={{ opacity: 0, y: 18, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.35 }} className="w-full max-w-md rounded-[34px] border border-white/70 bg-white/65 p-6 shadow-2xl backdrop-blur-2xl dark:border-white/10 dark:bg-slate-900/70 sm:p-8">
          <div className="mb-7 text-center">
            <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-[#7B1824] text-white shadow-xl shadow-[#7B1824]/20 dark:shadow-[#64131D]/30"><span className="text-xl font-black">SF</span></div>
            <h1 className="text-2xl font-black tracking-tight text-slate-900 dark:text-white">Store<span className="text-rose-800 dark:text-rose-200">Flow</span></h1>
            <p className="mt-1 text-sm font-medium text-slate-400">Panel Admin AIME-Dimsum</p>
          </div>

          <div className="mx-auto mb-6 h-1.5 w-20 rounded-full bg-[#7B1824]" />
          <div className="mb-6">
            <p className="text-[10px] font-black uppercase tracking-[0.22em] text-rose-800 dark:text-rose-200">Admin</p>
          ...
          <form className="space-y-4" onSubmit={submit}>
            ...
            <div className="flex flex-col gap-2 pt-2 sm:flex-row">
              <button type="button" onClick={() => nav('/')} className="order-2 flex-1 rounded-2xl border border-white/70 bg-white/70 px-4 py-3.5 text-sm font-black text-slate-700 shadow-sm transition hover:-translate-y-0.5 hover:border-indigo-200 hover:text-rose-800 dark:border-white/10 dark:bg-white/5 dark:text-slate-200 dark:hover:border-rose-400/30 sm:order-1">Kembali</button>
              <button type="submit" disabled={loading} className="order-1 flex-1 rounded-2xl bg-[#7B1824] px-4 py-3.5 text-sm font-black text-white shadow-xl shadow-[#7B1824]/20 transition hover:-translate-y-0.5 hover:bg-[#64131D] disabled:cursor-not-allowed disabled:opacity-60 dark:shadow-[#64131D]/30 sm:order-2">{loading ? 'Memproses...' : 'Sign In'}</button>
            </div>
          </form>
        </motion.div>
      </div>
